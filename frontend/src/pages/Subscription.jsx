    ...
    <div className="min-h-screen pt-28 pb-20 px-6 md:px-12">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className={`relative bg-gradient-to-b ${plan.color} border-2 ${plan.border} rounded-2xl p-8 flex flex-col transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-primary/20`}
          >
            {plan.badge && (
            ...
            <button
              ...
              className={`w-full py-3 rounded-xl font-bold transition-all ${plan.id === 'standard' ? 'bg-primary hover:bg-red-700 text-white' : 'bg-secondary/80 hover:bg-secondary text-white border border-white/20'}`}
            >
              Get {plan.name}
            </button>
          </div>
